...
class BackupCog(commands.Cog):
    ...
    def __init__(self, bot):
        self.bot = bot; self.last_backup = None; self.last_error = None; self.backup_loop.start()
    def cog_unload(self): self.backup_loop.cancel()
    async def _make_backup(self):
        ...
        self.last_backup, self.last_error = path, None
        return path
    @tasks.loop(seconds=BACKUP_INTERVAL_SECONDS)
    async def backup_loop(self):
        try: logger.info("Automatisches Backup erstellt: %s", await self._make_backup())
        except Exception as exc: self.last_error = str(exc); logger.exception("Automatisches Backup fehlgeschlagen", exc_info=exc)
    @backup_loop.before_loop
    async def before_backup_loop(self):
        await self.bot.wait_until_ready()
        try: logger.info("Startup-Backup erstellt: %s", await self._make_backup())
        except Exception as exc: self.last_error = str(exc); logger.exception("Startup-Backup fehlgeschlagen", exc_info=exc)
    @backup.command(name="now", description="Erstellt sofort ein Backup")
    async def backup_now(self, interaction):
        if not await self.bot.is_owner(interaction.user): return await interaction.response.send_message("❌ Nur der Bot-Owner darf Backups auslösen.", ephemeral=True)
        await interaction.response.defer(ephemeral=True)
        try: path = await self._make_backup()
        except Exception as exc: logger.exception("Manuelles Backup fehlgeschlagen", exc_info=exc); return await interaction.followup.send(f"❌ Backup fehlgeschlagen: `{str(exc)[:300]}`", ephemeral=True)
    ...
    async def backup_status(self, interaction):
        if not await self.bot.is_owner(interaction.user): return await interaction.response.send_message("❌ Nur der Bot-Owner darf den Backup-Status sehen.", ephemeral=True)
        ...
        await interaction.response.send_message(f"**Backup-Status**\nIntervall: `{BACKUP_INTERVAL_SECONDS // 60} Min.`\nVorhandene Backups: `{len(files)}`\nLetztes Backup: `{latest}`\nFehler: `{(self.last_error or 'Keiner')[:500]}`", ephemeral=True)
