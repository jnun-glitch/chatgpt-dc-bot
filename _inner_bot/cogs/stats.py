"""Statistiken: Command-Nutzung, System-Metriken (psutil), Mitglieder-Wachstum, Leveling."""
...
from core.db import (
    track_command_usage, get_command_usage, save_member_snapshot,
    get_member_history, _add_xp, _get_xp, _get_leaderboard
)
...
class StatsCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        lu_text = format_msg(message.guild.id, 'levelup_msg',
        ...
        if earned_role:
            try:
                ...
                lr_text = format_msg(message.guild.id, 'levelup_role_msg',
                                     mention=message.author.mention,
                                     name=message.author.display_name,
                                     level=new_level, role=earned_role.name)
    ...
    @app_commands.command(name='stats', description='Zeigt Bot-Statistiken und die meistgenutzten Commands')
    async def cmd_stats(self, interaction: discord.Interaction):
        ...
        lines = [f'{i+1}. `/{c["command"]}` – {c["used"]}x' for i, c in enumerate(usage)]
        if not lines:
            lines = ['Noch keine Daten.']
    ...
    @app_commands.command(name='system', description='Zeigt System-Ressourcen (CPU, RAM, Disk)')
    ...
    @app_commands.command(name='growth', description='Zeigt das Mitglieder-Wachstum des Servers')
    async def cmd_growth(self, interaction: discord.Interaction):
        ...
        if len(history) < 2:
            embed = discord.Embed(
                ...
                description=f'Aktuell: **{interaction.guild.member_count}** Mitglieder.\n'
                            'Weitere Datenpunkte sammeln sich im Laufe der Zeit.',
        ...
        for h in history:
            height = max(1, int(8 * h['count'] / max_count))
            bars.append('█' * height)
        ...
        embed = discord.Embed(
            title='📈 Mitglieder-Wachstum',
            description=f'{arrow} Von **{first}** auf **{last}** ({delta:+d})',
            color=discord.Color.blue()
        )
        embed.add_field(name='Verlauf (letzte 14)', value='```\n' + '\n'.join(bars) + '\n```', inline=False)
        await interaction.response.send_message(embed=embed)

    @app_commands.command(name='leaderboard', description='Zeigt die Top-Spieler nach Level und XP')
    ...
    async def cmd_leaderboard(self, interaction: discord.Interaction, limit: int = 10):
        ...
        if not entries:
            embed = discord.Embed(title='🏆 Leaderboard', description='Noch keine Daten vorhanden.', color=discord.Color.gold())
            await interaction.response.send_message(embed=embed)
            return
        medals = ['🥇', '🥈', '🥉']
        lines = []
