...
YouTube and Twitch are handled with persistent event state so an event is
announced once even when the bot restarts. The first poll seeds the current
YouTube feed without replaying old uploads.
"""
...
def youtube_video_state(video_id: str) -> dict:
    """Best-effort classification of a YouTube watch page."""
    try:
...
def x_user(username: str) -> dict:
    ...
    data = _http_json(
        f"https://api.x.com/2/users/by/username/{urllib.parse.quote(username.lstrip('@'))}",
        headers=headers,
    ).get("data")
    if not data:
...
def x_posts(user_id: str, since_id: str | None = None) -> list[dict]:
    ...
    headers = {"Authorization": f"Bearer {X_BEARER_TOKEN}", "User-Agent": "ScratchAI/1.0"}
    payload = _http_json(
        "https://api.x.com/2/users/{}/tweets?{}".format(user_id, urllib.parse.urlencode(params)),
        headers=headers,
    )
...
class SocialCog(commands.Cog):
    ...
    async def _youtube(self, sub: Subscription):
        ...
        if not sub.last_seen:
            # Seed current feed without replaying old uploads.
            # If a creator is already live, announce that live event once.
            for item in reversed(items):
                state = await asyncio.to_thread(youtube_video_state, item["id"])
                if youtube_kind(state) == "live":
                    await self._send(sub, self._youtube_live_embed(item))
